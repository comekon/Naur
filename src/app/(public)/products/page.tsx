...
export default function ProductsPage() {
  ...
  const filtered = useMemo(
    () => filterProducts({ category: category || undefined, teaType: teaType || undefined, origin: origin || undefined, sort: sort as "newest" | "bestseller" | "price-asc" | "price-desc" }),
    ...
    <>
      ...
      <section className="bg-secondary">
        <div className="mx-auto max-w-[1280px] px-6 py-12 md:px-8">
          <h1 className="font-display text-5xl font-bold text-text-light">Produk</h1>
        </div>
      </section>
        ...
        <div className="flex flex-col gap-8 md:flex-row">
          ...
          <div className="flex-1">
            ...
            <div className="mb-6 flex items-center gap-3">
              <span className="text-sm font-bold text-text-muted">Urutkan:</span>
              {sortOptions.map((opt) => (
                <button
                  key={opt.value}
            ...
            {filtered.length === 0 ? (
              <EmptyState
                ...
                action={
                  <button
                    onClick={() => { setCategory(""); setTeaType(""); setOrigin(""); }}
