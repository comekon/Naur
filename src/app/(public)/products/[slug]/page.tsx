...
export default function ProductDetailPage() {
    ...
    return (
      <SectionContainer>
        <div className="py-20 text-center">
          <h1 className="font-display text-4xl font-bold">Produk tidak ditemukan</h1>
          <Link href="/products" className="mt-6 inline-block font-display font-bold text-primary hover:underline">
  ...
  const optionButtons = (options: { value: string; label: string }[], selected: string, onChange: (v: string) => void) =>
    options.map((opt) => (
      <button
        key={opt.value}
  ...
  const tabs = [
    {
      label: "Deskripsi",
      content: <p className="text-text-muted leading-relaxed">{product.description}</p>,
    },
    {
      label: "Cara Seduh",
      content: <p className="text-text-muted leading-relaxed">{product.brewGuide}</p>,
    },
    {
      label: "Review",
      content: (
        <div className="space-y-4">
          {[
            ...
            { name: "Budi", rating: 4, text: "Tehnya berkualitas, packaging-nya juga keren." },
            { name: "Sari", rating: 5, text: "Favorite baru gue. Recommended!" },
          ].map((review) => (
            <div key={review.name} className="border-b-2 border-text-dark/10 pb-4">
    ...
    <>
      ...
      <section className="bg-secondary">
        <div className="mx-auto max-w-[1280px] px-6 py-4 md:px-8">
          <Link href="/products" className="font-display text-sm font-bold text-text-light hover:underline">
            &larr; Katalog Produk
          </Link>
        </div>
      </section>
            ...
            <div className="relative flex aspect-square items-center justify-center overflow-hidden rounded-flat border-2 border-text-dark bg-accent/15">
              {product.images[0] ? (
                ...
                <>
                  ...
                  <svg viewBox="0 0 120 120" fill="none" className="relative h-40 w-40 opacity-30">
                    <rect x="15" y="25" width="90" height="70" rx="8" stroke="#1A1A1A" strokeWidth="3" />
                    <circle cx="60" cy="60" r="18" stroke="#1A1A1A" strokeWidth="2.5" />
                    <path d="M50 55 L57 62 L72 47" stroke="#E63000" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                  <p className="absolute bottom-6 font-label text-xs text-text-muted opacity-50">Foto produk</p>
                </>
              )}
            </div>
          </div>
          ...
          <div className="md:w-1/2">
            ...
            {product.category === "Minuman RTD" && (
              <div className="mt-8 space-y-5">
                <div>
                  <p className="font-display text-sm font-bold mb-2">Sugar Level</p>
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              <FlatButton variant="solid" color="primary" size="lg" onClick={addToCart}>
                {added ? "Ditambahkan!" : "Tambah ke Keranjang"}
              </FlatButton>
              <FlatButton variant="solid" color="dark" size="lg" onClick={buyNow}>
                Beli Sekarang
              </FlatButton>
            </div>
